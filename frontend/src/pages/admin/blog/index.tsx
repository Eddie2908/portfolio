...
export default function AdminBlog() {
  ...
  const columns = [
    ...
    { key: 'category', label: 'Catégorie', render: (v) => v ? <span className="tag text-xs">{v}</span> : '-' },
    {
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <Link href={`/admin/blog/edit/${row.id}`} aria-label="Modifier" className="p-1.5 rounded-lg hover:bg-white/10 text-white/50 hover:text-white transition-colors">
            <Edit size={14} />
          </Link>
    ...
    <AdminLayout title="Blog">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <p className="text-white/50 text-sm">{loading ? '…' : `${posts.length} articles`}</p>
